...
public final class PinBlockTDEAEncrypter {
  ...
  private static final String PIN_ENCRYPTION_KEYSTORE_PATH =
      "mastercard.issuing.client.ref.app.pin.encryption.tdea.public.key.file";

  /** The Constant PIN_ENCRYPTION_ALGO. */
  private static final String PIN_ENCRYPTION_ALGO =
      "astercard.issuing.client.ref.app.pin.encryption.algorithm";
   ...
   */
  public EncryptedPinBlock encryptPin(String pin, String cardNumber)
      throws ReferenceAppGenericException {
    ...
    /**
     * Step 2: Generate double length TDEA (Triple DES Encryption Algorithm) key (Session key
     * generation same as payload encryption key except key length difference)
     */
    ...
    try {
      encryptedPin =
          Hex.encodeHexString(encryptData(desKey, Hex.decodeHex(pinBlock.toCharArray())))
              .toUpperCase();
    ...
    /**
     * Step 4: Encode double length 3DES encryption key (created in step 1) in ASN.1 DER format
     * (Same as payload encryption process)
     */
    /** pass key length as 112 (double length) and the Hex encoded DES key */
    String derEncodedDesKey = derEncodeWithFixIV(112, desKey);

    /**
     * Step 5: Encrypt ASN.1 DER encoded DES key under the recipient’s RSA public key and hex encode
     * it (Same as payload encryption process)
     */
    ...
    /** Hex.decodeHex - Converts an array of characters representing hexadecimal values into 
    ...
    try {
      encryptedKeyBytes =
          encryptKey(
              rsaPublicKey,
              Hex.decodeHex(null != derEncodedDesKey ? derEncodedDesKey.toCharArray() : null));
   ...
   * @throws Exception the exception
   */
  public String generatePinBlock(String pin, String cardNumber)
      throws ReferenceAppGenericException {
  ...
  public String generateDesKey(int length) {
    ...
    try {
      ...
      key = Hex.encodeHexString(keyGenerator.generateKey().getEncoded()).toUpperCase();
   ...
   */
  public byte[] encryptData(String desKey, byte[] plainText)
    ...
    cipher.init(
        Cipher.ENCRYPT_MODE, new SecretKeySpec(Hex.decodeHex(desKey.toCharArray()), "DESede"));
  ...
  public String derEncodeWithFixIV(int keyLength, String desKey) {
    ...
    if (keyLength == 112) {
      derEncodedDesKey =
          "30240410" + desKey.substring(0, 32) + "041099999999999999999999999999999999";
  ...
  public PublicKey getRSAPublicKey(String file) throws IOException, GeneralSecurityException {
    ...
    if (url == null) {
      throw new IOException("Unable to find " + file + " in classpath");
    }
    ...
    try {
      ...
      throw new IOException("Unable to find " + url.getPath() + " in classpath");
    }
